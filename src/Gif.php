...
class Gif
{
     ...
     * 构造方法，用于解码GIF图片
     *
     * @param string|null $src GIF图片数据
     * @param string      $mod 图片数据类型
     * @throws Exception
     */
    public function __construct(string $src = null, string $mod = 'url')
    {
        if (!is_null($src)) {
     ...
     * 设置或获取当前帧的数据
     *
     * @param string|null $stream 二进制数据流
     * @return mixed|void        获取到的数据
     */
    public function image(string $stream = null)
    {
        if (is_null($stream)) {
     ...
     * @return string 当前帧数据
     */
    public function nextImage(): string
    {
